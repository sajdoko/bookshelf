    ...
    <section class="container text-center search-hero-browse">
        <div class="row">
            <div class="col-12">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["REQUEST_URI"]); ?>" method='post' class='row gx-3 gy-2 align-items-center'>
                    <div class='col-sm-3'>
                        <label class='visually-hidden' for='searchTitle'>Title</label>
                        <input type='text' name='title' value="<?php echo $_POST["title"] ?? ''; ?>" class='form-control' id='searchTitle'
                               placeholder='Title'>
                    </div>
                    <div class='col-sm-3'>
                        <label class='visually-hidden' for='searchAuthor'>author</label>
                        <input type='text' name='author' value="<?php echo $_POST["author"] ?? ''; ?>" class='form-control' id='searchAuthor'
                               placeholder='Author'>
                    </div>
                    <div class='col-sm-2'>
                        <label class='visually-hidden' for='searchLanguage'>Language</label>
                        <input type='text' name='language' value="<?php echo $_POST["language"] ?? ''; ?>" class='form-control' id='searchLanguage'
                               placeholder='Language'>
                    </div>
                    <div class='col-sm-2'>
                        <label class='visually-hidden' for='searchPublisher'>Publisher</label>
                        <input type='text' name='publisher' value="<?php echo $_POST["publisher"] ?? ''; ?>" class='form-control' id='searchPublisher'
              ...
              <?php if (isset($books) && count($books) > 0) : ?>
                  <table class="table mt-5">
                      ...
                      <?php foreach ($books as $book) : ?>
                          <tr>
                              <td><a href='/book/<?php echo create_url_string($book['Boo_Title']); ?>'
                                     title="<?php echo $book['Boo_Title']; ?>"><?php echo $book['Boo_Title']; ?></a></td>
