  ...
  if (isset($_POST['submitRegister'])) {
    ...
    if (!$email) {
      ...
      if (!$password) {
        ...
        $country_id = filter_input(INPUT_POST, 'country', FILTER_SANITIZE_NUMBER_INT);
        ...
        if ($user) {
          ...
          if (!executeQuery($query, [$first_name, $last_name, $email, $phone, 3, $password])) {
            $error_msg .= '<p class="text-danger">Registration failure: INSERT</p>';
          } else {
            $last_id = retrieveOneRow('SELECT SCOPE_IDENTITY() as last_inserted_id');
            executeQuery(
                    'INSERT INTO ADDRESS VALUES (?, ?, ?, ?, ?, ?)',
                    [$last_id['last_inserted_id'], $street, $email, $zip, $city, $country_id]
            );
    ...
    <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
        ...
        <div class='row'>
            ...
            <div class='col-md-3'>
                ...
                <input type='text' name="zip" class='form-control' id='zip' placeholder='00000' required>
            </div>
            <div class='col-md-6'>
                <label for='city' class='form-label'>City</label>
                <input type='text' name='city' class='form-control' id='city' placeholder='California' required>
            </div>
            <div class='col-md-6'>
